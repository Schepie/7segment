...
static void showWs2812SpiDma() {
  ...
  // Clock in 1 dummy blank pixel (24 bits) to ensure the very last physical LED
...
void addPadelPoint(int team) {
  ...
  if (gameWon) {
    ...
    // Padel: Win set at 6 games with >=2 lead, or tiebreak at 7 games, or pro-set at 9 games
    bool setWon = false;
    if (gameWinner == 1) {
...
void setup() {
  ...
  Serial.println("\n=======================================================");
  Serial.println("   ESP32 Padel Scoreboard - Sets & Match Edition        ");
  ...
  splashText("SCAN", pixels.Color(0, 180, 255));
  delay(600);
