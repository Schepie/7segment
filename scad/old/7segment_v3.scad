...
// Butterfly key profile (classic dovetail dogbone)
module butterfly_profile(clearance = 0) {
    c = clearance;
    // Length: 24mm (X from -12 to 12)
    // Width at ends: 16mm (Y from -8 to 8)
    // Width at waist: 8mm (Y from -4 to 4)
    polygon([
        [-12 - c, 8 + c],
        [0, 4 + c],
        [12 + c, 8 + c],
...
// Generates the physical keys for printing
module print_butterfly_keys() {
    // Generates 2 keys side-by-side
    for(i=[0, 1]) {
        translate([0, i * 20, 0])
            linear_extrude(3.0) // 3mm thick to perfectly fill the pocket
                butterfly_profile(clearance=-0.15); // Slightly undersized for easy insertion
    }
}
...
module frontplate_black() {
    difference() {
        union() {
            ...
            // Old mounting bosses removed for v3 screwless design.
...
module backplate() {
    difference() {
        ...
        // Butterfly key recesses on the backplate (Z = outside face)
        for (y = [digit_height/3, -digit_height/3]) {
            // Right side (only if not the last panel)
            if (panel_id < 4) {
                translate([digit_width/2, y, -backplate_thick - 0.1])
                    linear_extrude(3.1) // 3mm deep pocket
                        butterfly_profile(clearance=0);
            }
            // Left side (only if not the first panel)
            if (panel_id > 1) {
                translate([-digit_width/2, y, -backplate_thick - 0.1])
                    linear_extrude(3.1)
                        butterfly_profile(clearance=0);
...
translate([digit_width/2 + 25, 0, -20]) color("Orange") print_butterfly_keys();
