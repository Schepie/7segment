...
// --- Preview & Inspection Controls ---
/* [Inspection & Quality Control] */
inspection_mode    = "assembled"; // ["assembled": Fully Assembled Clock, "collision_check": Highlight Inter-Module Collisions in Red, "cutaway_x": Cutaway View (Y-Z Plane), "cutaway_y": Cutaway View (X-Z Plane)]
cutaway_depth      = 0.0;         // [-300:1:300] Cut offset in mm
...
alpha_diffusers        = 1.0; // [0.0:0.05:1.0]
alpha_backplates       = 1.0; // [0.0:0.05:1.0]
alpha_pogo_pins        = 1.0; // [0.0:0.05:1.0]
alpha_brackets         = 1.0; // [0.0:0.05:1.0]
...
module render_pogo_junction(seam_x) {
    translate([seam_x, 0, pogo_z]) {
        ...
        translate([-1.8, 0, 0])
            pogo_connector_model(is_male = true, alpha = alpha_pogo_pins);
            
        // Right side connector (receiving on right module, female target pads)
        translate([1.8, 0, 0])
            rotate([0, 180, 0])
                pogo_connector_model(is_male = false, alpha = alpha_pogo_pins);
    }
}
...
// =============================================================
// MAIN ASSEMBLY & INSPECTION
// =============================================================

module clock_full_geometry() {
    // 1. Digits & Colon
...
if (inspection_mode == "collision_check") {
    // Check for interference between adjacent abutting panels
    show_collision() {
        render_digit(1, x_digit1);
        render_digit(2, x_digit2);
    }
