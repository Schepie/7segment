...
alpha_pogo_connectors  = 1.0; // [0.0:0.05:1.0]
...
module render_padel_scoreboard() {
    ...
    translate([x_d1, 0, 0]) {
        if (show_frontplate_black) color("#1e293b", alpha_frontplate_black) frontplate_black(panel_id = 1);
        if (show_frontplate_white) translate([0, 0, exp_w_z]) color("#f8fafc", alpha_frontplate_white) frontplate_white();
        if (show_backplate) translate([0, 0, exp_b_z]) color("#334155", alpha_backplate) backplate(panel_id = 1);
        if (show_pogo_connectors) {
            // Right pogo
            translate([digit_w/2 - pogo_boss_depth + 1.0, 0, pogo_z]) pogo_connector_model(is_male = true, alpha = alpha_pogo_connectors);
        }
    }
    
    // 2. DIGIT 2 (Team 1 Second Digit, Panel 2 with Pogo on both left and right walls)
    translate([x_d2, 0, 0]) {
        if (show_frontplate_black) color("#1e293b", alpha_frontplate_black) frontplate_black(panel_id = 2);
        if (show_frontplate_white) translate([0, 0, exp_w_z]) color("#f8fafc", alpha_frontplate_white) frontplate_white();
        if (show_backplate) translate([0, 0, exp_b_z]) color("#334155", alpha_backplate) backplate(panel_id = 2);
        if (show_pogo_connectors) {
            // Left pogo
            translate([-digit_w/2 + pogo_boss_depth - 1.0, 0, pogo_z]) rotate([0, 180, 0]) pogo_connector_model(is_male = false, alpha = alpha_pogo_connectors);
            // Right pogo
            translate([digit_w/2 - pogo_boss_depth + 1.0, 0, pogo_z]) pogo_connector_model(is_male = true, alpha = alpha_pogo_connectors);
        }
    }
    ...
    translate([x_gs, 0, 0]) {
        ...
        if (show_frontplate_white) translate([0, 0, exp_w_z]) color("#f8fafc", alpha_frontplate_white) games_sets_frontplate_white();
        if (show_backplate) translate([0, 0, exp_b_z]) color("#334155", alpha_backplate) games_sets_backplate();
        if (show_pogo_connectors) {
            // Left pogo
            translate([-games_w/2 + pogo_boss_depth - 1.0, 0, pogo_z]) rotate([0, 180, 0]) pogo_connector_model(is_male = false, alpha = alpha_pogo_connectors);
            // Right pogo
            translate([games_w/2 - pogo_boss_depth + 1.0, 0, pogo_z]) pogo_connector_model(is_male = true, alpha = alpha_pogo_connectors);
        }
    }
    
    // 4. DIGIT 3 (Team 2 First Digit, Panel 3 with Pogo on both left and right walls)
    translate([x_d3, 0, 0]) {
        if (show_frontplate_black) color("#1e293b", alpha_frontplate_black) frontplate_black(panel_id = 3);
        if (show_frontplate_white) translate([0, 0, exp_w_z]) color("#f8fafc", alpha_frontplate_white) frontplate_white();
        if (show_backplate) translate([0, 0, exp_b_z]) color("#334155", alpha_backplate) backplate(panel_id = 3);
        if (show_pogo_connectors) {
            // Left pogo
            translate([-digit_w/2 + pogo_boss_depth - 1.0, 0, pogo_z]) rotate([0, 180, 0]) pogo_connector_model(is_male = false, alpha = alpha_pogo_connectors);
            // Right pogo
    ...
    translate([x_d4, 0, 0]) {
        if (show_frontplate_black) color("#1e293b", alpha_frontplate_black) frontplate_black(panel_id = 4);
        if (show_frontplate_white) translate([0, 0, exp_w_z]) color("#f8fafc", alpha_frontplate_white) frontplate_white();
        if (show_backplate) translate([0, 0, exp_b_z]) color("#334155", alpha_backplate) backplate(panel_id = 4);
        if (show_pogo_connectors) {
            // Left pogo
            translate([-digit_w/2 + pogo_boss_depth - 1.0, 0, pogo_z]) rotate([0, 180, 0]) pogo_connector_model(is_male = false, alpha = alpha_pogo_connectors);
        }
