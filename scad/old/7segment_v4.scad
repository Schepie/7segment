...
// --- Key Parameters ---
panel_id = 4;           // Set to 1, 2, 3, or 4 (Panel 4 is automatically the last panel)
...
module snap_clip() {
    ...
    polygon([
        ...
        [14, -1.5],     // arm inner
        [17, -1.5],     // tip inner
        [17, 0],        // tip outer (blunt for pushing)
        [14, 3],        // catch lip point
        [13, 1.5],      // catch base
        [0, 1.5]        // base outer
    ]);
}

// The solid block for the receiver (added to the inside of the left wall)
module snap_receiver_block() {
    translate([8, 0, 0]) cube([16, 10, 12], center=true);
}

// The hole cut out of the receiver block AND the outer wall
module snap_receiver_hole() {
    // Main insertion shaft (allows clip base and room to flex down)
    translate([8, -0.7, 0]) cube([18, 4.6, 10.5], center=true);
    
    // Catch expansion: allows the lip to snap back up and lock against the X=13 wall
    translate([15, 2.5, 0]) cube([4, 5, 10.5], center=true);
}
...
module frontplate_black() {
    difference() {
        union() {
            ...
            // Snap-fit male clips (Right side)
            if (panel_id < 4) {
                for (y = [digit_height/3, -digit_height/3]) {
                    // Top clip faces +Y, bottom clip faces -Y
                    translate([digit_width/2, y, tunnel_depth/2]) 
                        if (y > 0) { snap_clip(); } else { mirror([0,1,0]) snap_clip(); }
                }
            }
            
            // Snap-fit female receiver blocks (Left side)
            if (panel_id > 1) {
                for (y = [digit_height/3, -digit_height/3]) {
                    // Orientation matches the incoming clip
                    translate([-digit_width/2, y, tunnel_depth/2]) 
                        if (y > 0) { snap_receiver_block(); } else { mirror([0,1,0]) snap_receiver_block(); }
                }
            }
            
            // Inter-panel alignment protrusion (Right side, robust 12x12 male flange sticking out 4mm)
            // This acts as a massive shear pin to prevent the panels from moving up/down or front/back!
            if (panel_id < 4) {
                translate([digit_width/2, 0, tunnel_depth/2]) 
                    cube([8, 12.0, 12.0], center=true);
            }
        }
        ...
        // Old side mounting holes removed for v3 screwless design.
        
        // Snap-fit female receiver holes (Left side)
        // Cuts the insertion slot through the 2mm outer wall and creates the internal catch geometry
        if (panel_id > 1) {
            for (y = [digit_height/3, -digit_height/3]) {
                translate([-digit_width/2, y, tunnel_depth/2]) 
                    if (y > 0) { snap_receiver_hole(); } else { mirror([0,1,0]) snap_receiver_hole(); }
            }
        }
